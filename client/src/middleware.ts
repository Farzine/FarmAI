...
export async function middleware(request: NextRequest) {
  ...
  if (pathname == "/login" || pathname == "/expert/login") {
  ...
  const adminProtectedRoutes = ["/expert/dashboard"];
  ...
  if (adminProtectedRoutes.includes(pathname) && user.role == "User") {
    return NextResponse.redirect(
      new URL(
        "/expert/login?error=Please login first to access this route.",
        request.url
      )
    );
  }
  ...
  if (userProtectedRoutes.includes(pathname) && user.role == "Expert") {
    return NextResponse.redirect(
      new URL(
        "/login?error=Please login first to access this route.",
        request.url
