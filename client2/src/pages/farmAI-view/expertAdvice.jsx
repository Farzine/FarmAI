...
const ExpertAdvice = () => {
  ...
  useEffect(() => {
    if (expertAdviceList.length > 0) {
      setSelectedAdvice(expertAdviceList[0]); // Set the first advice as default
    }
  }, [expertAdviceList]);
      ...
      if (/^\d+\.\s.*?:$/.test(part)) {
        return (
          <div key={index} className="mt-2 block">
            ...
            {/* Bold the number and text before the colon */}
            <br /> {/* Start a new line */}
          </div>
        );
      }
            ...
            expertAdviceList.map((advice) => (
              ...
              >
                ...
                <div className="flex-grow pr-4">
                  ...
                  <h3 className="text-sm font-semibold whitespace-nowrap overflow-hidden text-ellipsis max-w-[15rem]">
                    {" "}
      ...
      <div className="w-3/4 bg-white p-8 overflow-y-auto scrollbar-hide">
        {selectedAdvice ? (
          <div>
            ...
            <div className="text-md mb-4 mt-8">
              {formatDescription(selectedAdvice.description)}
            </div>
          </div>
        ) : (
          <p className="text-lg text-gray-500">
            Please select an advice to view details.
          </p>
        )}
      </div>
    </div>
